function [ Also_Output_List, Eq_Network_List, residuals, x_star, Payoffs, Eq_Mat_List, Other_Agg_Outputs, Eq_Efficiency, Efficiencies, ...
   CP_Also_Output_List, CP_Eq_Networks, CP_Eq_Efficiency, edge_price, consumer_prices ] = Model_Simulation_PW_and_CP_Mult(Adj,Prod)
...
for m = 1:num_mats
    ...
    %edge_price = Find_Prices_Two_Inputs(Eq_A, Z, a, 0.5, w_star);
    ...
    for j = 1:num_firms
        ...
        for c = 1:num_cus
            ...
            network_rev = temp_rev + edge_price(c,1)*X_Mat(c,5);
        end
        
        %calculate the cost of firm j's intermediate input
        %network_cost = p(e_j)*x(e_j)
        network_cost = edge_price(j,1)*X_Mat(j,5);
    ...
    q_j = Eq_z.*(edge_price(:,1).^(-a))*(w_star^a);
    Efficiencies(m,:) = q_j;
...
CP_Also_Output_List = -1;

%find the pwise equilibrium
Eq_Network_List = Pick_Equilibrium_Mult(Eq_Mat_List,Payoffs,sup_matrix);

%if you successfully find a pw equilibrium, find the appropriate values
if Eq_Network_List(1,1) ~= 0

    eq_index_list = find(ismember(Eq_Mat_List,Eq_Network_List,'rows'));


    %Eq_Output = Aggregate_Outputs(eq_index);
    Also_Output_List = Other_Agg_Outputs(eq_index_list);
    %Eq_Efficiency = Efficiencies(eq_index,:);

end

%find the cp equilibrium
CP_Eq_Networks = Find_CP_Eq_Mult(Eq_Mat_List, Payoffs, sup_matrix);

%if you successfully find a cp equilibrium, find the appropriate values
if CP_Eq_Networks(1,1) ~= 0
    
    %find the index
    cp_index_list = find(ismember(Eq_Mat_List,CP_Eq_Networks,'rows'));
    
    %get the associated values
    %CP_Eq_Output_list = Aggregate_Outputs(cp_index_list);
    CP_Also_Output_List = Other_Agg_Outputs(cp_index_list);
    %CP_Eq_Efficiency = Efficiencies(cp_index,:);  
