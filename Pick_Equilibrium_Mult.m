%This function determines which of the potential equilbrium networks are
%pairwise stable equilbria.
...
%   -This is a list of all equilibria


function [E] = Pick_Equilibrium_Mult(P,Pay,Sup)
...
%initialize E as a matrix of zeros num_options by num_firms
E = zeros(num_options, num_firms);
%create a counter to store where the next available row is.
storage_counter = 1;
...
while o <= num_options
    ...
    %If you found a deviation, increase o becuase the one you tried wasn't
    %an eq.
if found_dev == 1
    temp_o = o;
    o = temp_o + 1;
 %Otherwise you didn't find a deviation and this is a pairwise equilbirum
   %     set the next row of E to be current, incremenet the storage
   %     counter and increment o.
else
    E(storage_counter,:) = current;
    %increment storage counter
    temp_count = storage_counter;
    storage_counter = temp_count + 1;
    %increment o
    temp_o_2 = o;
    o = temp_o_2 + 1;
end
