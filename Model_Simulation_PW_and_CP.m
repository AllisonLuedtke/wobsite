function [ Also_Output, Eq_Network, residuals, x_star, Payoffs, Eq_Mat_List, Other_Agg_Outputs, Eq_Efficiency, Efficiencies, ...
   CP_Also_Output, CP_Eq_Network, CP_Eq_Efficiency ] = Model_Simulation_PW_and_CP(Adj,Prod)
...
Eq_Network = Pick_Equilibrium(Eq_Mat_List,Payoffs,sup_matrix);
...
CP_Eq_Network = Find_CP_Eq(Eq_Mat_List, Payoffs, sup_matrix);

%if you successfully find a cp equilibrium, find the appropriate values
if CP_Eq_Network(1) ~= -1
    
    %find the index
    cp_index = find(ismember(Eq_Mat_List,CP_Eq_Network,'rows'));
    
    %get the associated values
    CP_Eq_Output = Aggregate_Outputs(cp_index);
    CP_Also_Output = Other_Agg_Outputs(cp_index);
